...
nblock=15;  % used when states are not generated by HMM
% Parameter to generate precision matrices
n_perturb_add = [10,10,10,10,15,5,5];
n_perturb_delete = [10,10,10,10,15,5,5];
...
for k=2:S_true
    ...
    % Sample locations for 6 zeros and 6 nonzero entries without replacement
    % to perturb
    pos_inds = randsample(size(rpos, 1), n_perturb_delete(k-1));
    zero_inds = randsample(size(rzero, 1), n_perturb_add(k-1));

    % Add new and remove edge to get A2 from A1
...
% C_temp = C_true;
% for s=1:S_true
%     std_temp = unifrnd(.5,1.5,[p,1]);
%     C_temp(:,:,s) = diag(std_temp) * C_true(:,:,s) * diag(std_temp);
% end
% C_true=C_temp;
...
for rep = 1:nrep    
    ...
    %% Add state mean
    mu_true = zeros(p,S_true);
    % mu_true=normrnd(0,5,[p,S_true]);
    % for k=1:S_true
    %     TC(states_true==k,:)= TC(states_true==k,:)+  repmat( mu_true(:,k)',[sum(states_true==k),1]);
    % end
    ...
    data.S_true=S_true;
    data.C_true=C_true;
    data.n_perturb_add=n_perturb_add;
    data.n_perturb_delete=n_perturb_delete;
    data.states_true=states_true;
    data.nblock = nblock;
    data.mu_true=mu_true;
    data.Phi_true=Phi;
    data.method = 'mvnhmm';
    
    save(['Synthetic_data/Synthetic_data_mvnhmm_rep_', num2str(rep),'.mat'],'data')
