...
spikescale = 10; % standard deviation of the normal outlier

%% Generate precision matrices
% AR(2) graph in state 0
C_temp=toeplitz([1, 0.5, 0.4, zeros(1, p - 3)]);
...
for k=1:S_true
    ...
    pos_inds = randsample(size(rpos, 1), n_perturb_delete(k));
    zero_inds = randsample(size(rzero, 1), n_perturb_add(k));

    % Add new edges
    %for j = 1:n_perturb_add(k-1)
    for j = 1:n_perturb_add(k)
...
for rep = 1:nrep    
    ...
    save(['Synthetic_data_appendix/Synthetic_data_',num2str(nspikes),'spikeshmm_rep_', num2str(rep),'.mat'],'data')
