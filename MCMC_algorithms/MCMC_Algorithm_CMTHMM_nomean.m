function [C_save, adj_save, ppi_HMM, states_save, Phi_save, ar_Phi,...
    log_mh_ratio_Phi_save, Phi_prop_save, tau_t_save ]...
    = MCMC_Algorithm_CMTHMM_nomean(burnin, nmc, Y, ...
    S, ...  % number of hidden states
    v0, v1, pii,...  % parameters for graphical model
    nu, ...    % parameters for t-distribution
    Sig, C, adj, Phi,...  % initialization of graphical model
    states, ...   % initialization of HMM
    tau_t,...   % initialization for t-distribution
    disp_result)

% Details of inputs and outputs are in the function Call_Function_CMTHMM_nomean.m
...
for iter = 1: burnin + nmc
    ...
    a_gam= (nu+p)/2;
    for s=1:S
        b_gam= nu+ diag(yNorm(:,states==s)'* C(:,:,s)* yNorm(:,states==s));
        tau_t(states==s) = gamrnd(a_gam, 2./b_gam);    
    end
...
%% Udpate cross-graph correlation matrix Phi
    ...
    % V: D inverse in paper
    V=zeros(S,S);
    for s = 1:S
        temp=C(:, :, s) .^ 2;
        temp=temp(upperLogic(p));
    ...
    if (log(rand(1)) < log_mh_ratio_Phi) || (iter<10) % (iter==1)
        Phi = Phi_prop;
